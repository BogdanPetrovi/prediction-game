...
const PredictionHistoryCard:React.FC<RecentPredictions> = ({ predictedWinner, result, team1, team2, winnerTeam }) => {
  return (
    <div 
      ...
      } w-full lg:w-5/12 h-[4.5rem] bg-secondary flex items-center justify-between px-3 rounded-2xl border-2`}>
      {/* team1 */}
      <div className="flex items-center gap-0 md:gap-2 text-lg md:text-2xl font-bold">
        {
        ...
        <h3 className="w-min md:w-fit">{team1.name}</h3>
      </div>

      {/* result for finished or "vs" for upcoming */}
      {
        result ? 
        <h3 className="text-xl lg:text-4xl font-bold text-white">{result}</h3>
        :
        <h4 className="text-lg lg:text-2xl font-bold text-white/30">VS</h4>
      }

      {/* team2 */}
      <div className="flex items-center gap-0 md:gap-2 text-lg md:text-2xl font-bold">
        <h3 className="w-min md:w-fit">{team2.name}</h3>
